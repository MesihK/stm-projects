...
//available  commands
char * keyworld [] = {_CMD_HELP, _CMD_CLEAR, _CMD_LIST, _CMD_NAME, _CMD_VER, _CMD_LISP, _CMD_RE};
// version subcommands
...
int execute (int argc, const char * const * argv)
{
	...
	while (i < argc) {
		...
		} else if (strcmp (argv[i], _CMD_RE) == 0) {
			if (++i < argc) {
                int m = re_match("[0-9]+", argv[i]);
                if( m != -1)
